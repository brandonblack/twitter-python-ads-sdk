...
class Resource(object):
    ...
    def from_response(self, response):
        ...
        for name in self.PROPERTIES:
            ...
            if transform and transform == 'time' and response[name]:
                setattr(self, attr, dateutil.parser.parse(response[name]))
            if isinstance(response[name], int) and response[name] == 0:
                continue  # skip attribute
            else:
                setattr(self, attr, response.get(name, None))
...
class Persistence(object):
    ...
    def delete(self):
        ...
        response = Request(
            self.account.client(), 'delete', resource).perform()
